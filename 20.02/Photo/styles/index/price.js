...
async function updateAvailableTimes(selectedDate) {
    ...
    try {
        const response = await fetch('http://localhost:1234/get-reserved-times', {
            method: 'GET',
            headers: {
...
document.getElementById('submitButton').addEventListener('click', async function (e) {
    ...
    if (paymentMethod === 'cash') {
        const response = await fetch('http://localhost:1234/reserve', {
            method: 'POST',
...
async function handlePayment(price, reservationData, hcaptchaResponse) {
    try {
        ...
        const response = await fetch('http://localhost:1234/create-payment-intent', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (paymentIntent.status === 'succeeded') {
            ...
            showToast('Грешка при плащането, моля, опитайте отново.', 'error');
            return false;
        }
    } catch (error) {
